...
You can directly execute the program from the `src` folder by starting the `schnitzel.rb` script. For production use, bundle all parts of the program into one single file with `build.rb` and transfer the resulting file `schnitzel` to the target machine.
...
## Structure
  ...
  * `schnitzel` - Game itself (only file needed on the target machine). Build it with `src/build.rb`.
